...
const std::string View::funcs[] = {"cos",  "sin",  "tan", "acos", "asin",
                                   "atan", "sqrt", "ln",  "log"};
...
View::View(QWidget* parent) : QMainWindow(parent), ui(new Ui::View) {
  ...
  connect(ui->smartButtons, SIGNAL(buttonClicked(QAbstractButton*)), this,
          SLOT(UpdateExpressionWindow_(QAbstractButton*)));
  connect(ui->tabs1, SIGNAL(buttonClicked(QAbstractButton*)), this,
          SLOT(ChangeTab_(QAbstractButton*)));
  connect(ui->tabs2, SIGNAL(buttonClicked(QAbstractButton*)), this,
          SLOT(ChangeTab_(QAbstractButton*)));
  connect(ui->tabs3, SIGNAL(buttonClicked(QAbstractButton*)), this,
          SLOT(ChangeTab_(QAbstractButton*)));
  connect(ui->pushButton_equal, SIGNAL(clicked()), this,
          SLOT(SendDataToSmartModel_()));
  connect(ui->pushButton_PLOT, SIGNAL(clicked()), this, SLOT(BuildGraph_()));
  connect(ui->pushButton_undo, SIGNAL(clicked()), this,
          SLOT(ClearExpressionWindow_()));
  connect(ui->creditEqual, SIGNAL(clicked()), this,
          SLOT(SendDataToCreditModel_()));
  connect(ui->depositEqual, SIGNAL(clicked()), this,
          SLOT(SendDataToDepositModel_()));
  connect(ui->addDeposit, SIGNAL(clicked()), this,
          SLOT(AddNewDepositToTable_()));
  connect(ui->addWithdrawal, SIGNAL(clicked()), this,
          SLOT(AddNewWithdrawalToTable_()));
  connect(ui->removeDeposit, SIGNAL(clicked()), this,
          SLOT(RemoveDepositFromTable_()));
  connect(ui->removeWithdrawal, SIGNAL(clicked()), this,
          SLOT(RemoveWithdrawalFromTable_()));
}
...
void View::UpdateExpressionWindow_(QAbstractButton* button) {
  ...
  if (ui->expressionWindow->text() == '0' && text != ".")
...
void View::ClearExpressionWindow_() { ui->expressionWindow->setText("0"); }
...
void View::ChangeTab_(QAbstractButton* button) {
  ...
  } else if (button == ui->Deposit1 ||
             button == ui->Deposit2 && tab_index != 2) {
...
void View::SendDataToCreditModel_() {
  ...
  if (return_code)
    credit_data.rate = ui->percentRate->text().toDouble(&return_code);
  if (return_code)
    credit_data.period = ui->creditPeriod->text().toDouble(&return_code);
  if (return_code) {
    credit_data.type =
        ui->type_annuity->isChecked() ? kAnnuity : kDifferentiated;
...
void View::SendDataToDepositModel_() {
  ...
  if (return_code)
    deposit_data.tax_rate = ui->taxRate->text().toDouble(&return_code);
  ...
  if (return_code) {
    ...
    deposit_data.capitalization =
        ui->capitalization->isChecked() ? true : false;

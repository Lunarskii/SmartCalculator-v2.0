...
bool Model::AppendIfNumber_() {
  ...
  for (;
       ptr != str.end() && (isDec(*ptr) || isDot(*ptr) ||
                            (isSign(*ptr) && isExp(*(ptr - 1))) || isExp(*ptr));
       ++ptr) {
    value += *ptr;
  }
...
bool Model::AppendIfFunc_() {
  ...
  for (std::size_t i = 0; i < sizeof(functions) / sizeof(std::string); ++i) {
    if (!str.compare(std::distance(str.begin(), it), functions[i].size(),
                     functions[i])) {
      std::string temp;
      ...
      try {
        output.emplace(
            CalcFunctions_(kCos + i, std::stold(tempObj.GetResult())));
      } catch (const std::exception& e) {
        return false;
      }
...
void Model::AppendOp_() {
  ...
  while (operators.size() != 0 &&
         DetPriority_(prev) <= operators.top().priority) {
  ...
  while (it != str.end()) {
    if (*it == ' ') {
      ...
      AppendBracket_();
               ...
               (std::string(it, str.end()).size() >= 3 && isMod(it))) {
      AppendOp_();
    } else if (*it == ')') {
      Splice_();
    } else {
...
long double Model::CalcOperations_(long double& operand1, long double& operand2,
                                   int type) {
  if (type == kPlus) {
...
void Model::ToDouble_() {
  ...
  if (GetCountNumbersOfOperators_() !=
      output.size() - GetCountNumbersOfOperators_() - 1) {
    str = "ERROR";
  } else {
    while (output.size() > 1 || temp.size() > 0) {
      if (isOperator(output.top().type)) {
        SpliceOperators_(output, temp);
      } else if ((output.size() <= 1 && temp.size() > 0) ||
                 (output.top().type == kNumber && temp.top().type == kNumber)) {
...
std::string Model::GetResult() const { return str; }

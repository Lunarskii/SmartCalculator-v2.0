...
class Model {
 ...
 private:
  ...
  long double CalcOperations_(long double& operand1, long double& operand2,
                              int type);
  void MoveElement_(std::stack<StackNode>& src, std::stack<StackNode>& dest);
  void SpliceOperators_(std::stack<StackNode>& src,
                        std::stack<StackNode>& dest);
 ...
 public:
  Model(long double amount, int period, long double rate, long double tax_rate,
        long double frequency, bool capitalization);

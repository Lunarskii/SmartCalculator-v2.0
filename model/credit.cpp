...
void Model::CreditCalculator() {
  if (credit_data.type == kAnnuity) {
    ...
    long double x =
        rate * pow(rate + 1, static_cast<long double>(credit_data.period)) /
    ...
    long double rate =
        credit_data.amount / static_cast<long double>(credit_data.period);

    credit_result.max_payment =
        Round_((rate + credit_data.amount * credit_data.rate *
                           Avg::kAvgDaysInMonths / Avg::kAvgDaysInYear),
               2);
    credit_result.min_payment =
        Round_((rate + rate * credit_data.rate * Avg::kAvgDaysInMonths /
                           Avg::kAvgDaysInYear),
               2);
    credit_result.payment =
        Round_((credit_result.max_payment + credit_result.min_payment) / 2, 2);
  }
  credit_result.total_payment = Round_(
      credit_result.payment * static_cast<long double>(credit_data.period), 2);
  credit_result.overpayment =
      Round_((credit_result.total_payment - credit_data.amount), 2);
}
...
CreditResult Model::GetCreditResult() { return credit_result; }
